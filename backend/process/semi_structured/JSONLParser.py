...
class JSONLParser(SemiStructuredProcessor):
    ...
    def _get_description_prompt(self, content_preview: str) -> str:
        ...
        return f"""请用自然语言描述以下JSONL文件，描述将用于语义检索，请包含用户可能搜索的关键信息。

要求：
- 说明这是什么数据、每条记录代表什么（如：员工信息记录、商品数据、日志条目等）
- 包含记录中的字段名（用户可能搜索"包含xxx字段的记录"）
- 包含数据中出现的具体名称、人名、项目名、类别等（用户可能直接搜索这些名称）
- 用你向别人介绍这个文件内容时会用的自然语言来写
